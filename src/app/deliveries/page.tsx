...
export default function DeliveriesPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const [ticketRows, runRows, driverRows, vehicleRows, locationRows] = await Promise.all([
                  ...
                  <Card key={run.id} className="rounded-xl border-white/10 bg-[#111]">
                    <CardHeader className="border-b border-white/5">
                      <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <button onClick={() => setSelectedRun(run)} className="rounded-md border border-white/10 px-3 py-1.5 text-xs font-semibold text-zinc-200 hover:bg-white/10">Edit</button>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="p-4">
                      <div className="mb-3 grid grid-cols-2 gap-2 md:grid-cols-4">
                        ...
                        <div className="rounded-lg bg-[#171717] p-3"><p className="text-[10px] uppercase text-zinc-500">Minutes</p><p className="text-lg font-bold text-white">{run.estimatedDurationMinutes}</p></div>
